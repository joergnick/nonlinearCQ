...
#	
	...
	def scale_fft(self,A,N,T):
		...
		n_rows=len(A[:,0])
		n_columns=len(A[0,:])
		A_fft=1j*np.ones((n_rows,L))
		
		for j in range(0,n_rows):
			first_part=rho**(np.linspace(0,n_columns-1,n_columns))*A[j,:]
			A_fft[j,:]=np.concatenate((first_part,np.zeros(L-n_columns)),axis=0)
			#A_hat[j,:]=np.concatenate((rho**(np.linspace(0,len(A[0,:])-1,len(A[0,:])))*A[j,:],np.zeros(L-len(A[0,:])+1)),axis=0)
			A_fft[j,:]=np.fft.fft(A_fft[j,:])
		return(A_fft)
	...
	def rescale_ifft(self,A,N,T):
		...
		n_rows=len(A[:,0])
		n_columns=len(A[0,:])
		#ift_A=1j*np.ones((n_rows,n_columns))
		A_sol=np.zeros((n_rows,n_columns))
		for j in range(0,n_rows):
			A_sol[j,:]=np.real(np.fft.ifft(A[j,:])
			A_sol[j,:]=np.real(rho**(-np.linspace(0,n_columns-1,n_columns))*A_sol[j,0:n_columns])
